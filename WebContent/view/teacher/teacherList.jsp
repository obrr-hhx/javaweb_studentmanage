	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        columns: [[  
				{field:'chk',checkbox: true,width:50},
 		        ...
 		        {field:'tn',title:'工号',width:150, sortable: true},    
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							var data = {clazzId:clazzid, name:name, password:password, sex:sex, phone:phone, qq:qq};
	  ...
	  //下拉框通用属性
	  	...
	  	$("#editDialog").dialog({
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							var tn = $("#edit_tn").textbox("getText");
							var sex = $("#edit_sex").textbox("getText");
							var phone = $("#edit_phone").textbox("getText");
							var qq = $("#edit_qq").textbox("getText");
							var data = {id:id, clazzid:clazzid, name:name, tn:tn, sex:sex, phone:phone, qq:qq};
			...
			onBeforeOpen: function(){
				...
				//设置值
				$("#edit_tn").textbox('setValue', selectRow.tn);
				$("#edit_name").textbox('setValue', selectRow.name);
				$("#edit_sex").textbox('setValue', selectRow.sex);
				$("#edit_phone").textbox('setValue', selectRow.phone);
				$("#edit_qq").textbox('setValue', selectRow.qq);
				$("#edit_photo").attr("src", "PhotoServlet?method=getPhoto&type=3&tid="+selectRow.id);
	  ...
	  //搜索按钮监听事件
	  	$("#search-btn").click(function(){
	  		$('#dataList').datagrid('load',{
	  			teacherName: $('#search_teacher_name').val(),
	  ...
	  //上传按钮事件监听
	  	$("#upload-photo-Btn").click(function(){
			...
			$("#uploadForm").attr('action',action+'&tid='+$("#set_photo_id").val());
			$("#uploadForm").submit();
			setTimeout(function(){
				var message =  $(window.frames["photo_target"].document).find("#message").text();
				$.messager.alert("消息提醒",message,"info");
				
				$("#edit_photo").attr("src", "PhotoServlet?method=getPhoto&tid="+$("#set_photo_id").val());
			}, 1500)
		});
...
<body>
	...
	<div id="toolbar">
		...
		<div style="float: left;margin-top:4px;" class="datagrid-btn-separator" >&nbsp;&nbsp;姓名：<input id="search_teacher_name" class="easyui-textbox" name="search_teacher_name" /></div>
	...
	<div id="addDialog" style="padding: 10px;">  
   		<div style=" position: absolute; margin-left: 560px; width: 200px; border: 1px solid #EEF4FF" id="photo">
    		<img alt="照片" style="max-width: 200px; max-height: 400px;" title="照片" src="PhotoServlet?method=getPhoto&type=3" />
	    </div> 
	...
	<div id="editDialog" style="padding: 10px">
		<div style=" position: absolute; margin-left: 560px; width: 250px; height: 300px; border: 1px solid #EEF4FF">
	    	...
	    	<form id="uploadForm" method="post" enctype="multipart/form-data" action="PhotoServlet?method=SetPhoto" target="photo_target">
	    		<!--TeacherServlet?method=SetPhoto -->
	    		<input type="hidden" id="set_photo_id" name="tid" />
		    	<input class="easyui-filebox" name="photo" data-options="prompt:'选择照片'" style="width:200px;">
		    	<input id="upload-photo-Btn" class="easyui-linkbutton" style="width: 50px; height: 24px;" type="button" value="上传"/>
		    </form>
	    </div>   
    	<form id="editForm" method="post">
	    	...
	    	<table id="editTable" border=0 style="width:800px; table-layout:fixed;" cellpadding="6" >
	    		<tr>
	    			<td>工号:</td>
	    			<td>
	    				<input id="edit_tn" data-options="readonly: true" class="easyui-textbox" style="width: 200px; height: 30px;" type="text" name="tn" data-options="required:true, missingMessage:'请输入工号'" />
	    			</td>
	    		</tr>
	    		<tr>
	    			<td>姓名:</td>
